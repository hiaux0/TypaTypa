...
export class CustomTable {
  ...
  attached() {
    ...
    this._bindingEventService.bind(
      gridContainerElm,
      "oncellchange",
      this.handleOnCellChange.bind(this),
    );
    this._bindingEventService.bind(
      gridContainerElm,
      "onvalidationerror",
    ...
    this._bindingEventService.bind(
      gridContainerElm,
      "onitemdeleted",
      this.handleItemDeleted.bind(this),
    );
    this._bindingEventService.bind(
      gridContainerElm,
    ...
    this.columnDefinitions = [
      {
        ...
        grouping: {
          getter: "title",
          formatter: (g) =>
            `Title: ${g.value} <span class="text-color-primary">(${g.count} items)</span>`,
      ...
      },
      {
        ...
        grouping: {
          getter: "duration",
          formatter: (g) =>
            `Duration: ${g.value} <span class="text-color-primary">(${g.count} items)</span>`,
          comparer: (a, b) => {
      ...
      },
      {
        ...
        grouping: {
          getter: "start",
          formatter: (g) =>
            `Start: ${g.value} <span class="text-color-primary">(${g.count} items)</span>`,
          aggregators: [new Aggregators.Sum("cost")],
          aggregateCollapsed: false,
          collapsed: false,
        },
      },
      {
        ...
        grouping: {
          getter: "finish",
          formatter: (g) =>
            `Finish: ${g.value} <span class="text-color-primary">(${g.count} items)</span>`,
          aggregators: [new Aggregators.Sum("cost")],
          aggregateCollapsed: false,
          collapsed: false,
        },
      },
      {
        ...
        grouping: {
          getter: "cost",
          formatter: (g) =>
            `Cost: ${g.value} <span class="text-color-primary">(${g.count} items)</span>`,
          aggregators: [new Aggregators.Sum("cost")],
      ...
      },
      {
        ...
        grouping: {
          ...
          formatter: (g) =>
            `% Complete:  ${g.value} <span class="text-color-primary">(${g.count} items)</span>`,
          aggregators: [new Aggregators.Sum("cost")],
          aggregateCollapsed: false,
          collapsed: false,
        },
          ...
          optionItems: [
            {
              option: true,
              title: "True",
              iconCssClass: "mdi mdi-check-box-outline",
            },
            {
              option: false,
              title: "False",
              iconCssClass: "mdi mdi-checkbox-blank-outline",
            },
          ],
        },
      },
  ...
  clearGroupingSelects() {
    this.selectedGroupingFields.forEach(
      (_g, i) => (this.selectedGroupingFields[i] = ""),
    );
  ...
  groupByDurationOrderByCount(sortedByCount = false) {
    ...
    const sortColumns = sortedByCount
      ? []
  ...
  toggleDraggableGroupingRow() {
    ...
    this.sgb?.slickGrid?.setTopHeaderPanelVisibility(
      !this.sgb?.slickGrid?.getOptions().showTopHeaderPanel,
    );
  }
  ...
  executeCommand(_e, args) {
    ...
    switch (command) {
      ...
      case "delete-row":
        if (
          confirm(
            `Do you really want to delete row (${args.row + 1}) with "${dataContext.title}"`,
          )
        ) {
